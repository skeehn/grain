...
function readBody(request: import('http').IncomingMessage): Promise<string> {
  return new Promise((resolve, reject) => { let value = ''; request.setEncoding('utf8'); request.on('data', chunk => { value += chunk; }); request.on('end', () => resolve(value)); request.on('error', reject); });
}

function card(title: string, body: string, kind = ''): string { return `<section class="card ${kind}"><h2>${esc(title)}</h2>${body}</section>`; }
function renderLab(runId?: string): string {
  ...
  const timeline = view.timeline.slice(-8).map(event => `<li><b>#${event.sequence}</b> ${esc(event.label)} <span>${esc(event.detail)}</span></li>`).join('');
  const files = view.workspace.length ? view.workspace.slice(-8).map(file => `<li>${esc(file.path)}</li>`).join('') : '<li>Nothing changed yet.</li>';
  const verify = `${view.diagnostics.passed} passed · ${view.diagnostics.failed} failed`;
  return [
    card('Rice report', `<div class="hero"><div class="rice" aria-label="Grain companion">(•ᴗ•)</div><div><strong>${esc(status)}</strong><p>${esc(view.run.task)}</p><small>${esc(view.run.provider)}/${esc(view.run.model)}</small></div></div>`, 'hero-card'),
    card('Work trace', `<ol>${timeline}</ol>`),
    card('Files', `<ul>${files}</ul>`),
    card('Verification', `<p class="metric">${esc(verify)}</p>${['running', 'paused', 'waiting_approval'].includes(status) ? `<button data-action="pause">${status === 'paused' ? 'Resume work' : 'Pause work'}</button>` : '<p class="muted">Viewing evidence only</p>'}`),
...
function shell(token: string, runId?: string): string {
  const theme = loadConfig().tui?.theme === 'light' ? 'studio' : (loadConfig().tui?.theme || 'field');
  return `<!doctype html><html data-theme="${esc(theme)}"><meta name="viewport" content="width=device-width"><title>Grain Lab</title>
<style>
...
export function startLabServer(port = 7332, host = '127.0.0.1', initialRunId?: string): Server {
  ...
  return createServer(async (request, response) => {
    ...
    if (url.pathname === '/') return send(200, shell(token, initialRunId || url.searchParams.get('run') || undefined));
    if (!authorized) return send(403, 'Grain Lab token required', 'text/plain');
    if (url.pathname === '/api/cards') return send(200, renderLab(url.searchParams.get('run') || initialRunId || undefined));
    let body: any = {}; try { body = JSON.parse(await readBody(request)); } catch { /* invalid body */ }
    ...
    if (url.pathname === '/api/action' && body.action === 'pause') {
      const id = body.runId || initialRunId || listRuns().at(-1); if (!id) return send(404, 'No run', 'text/plain');
