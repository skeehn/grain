...
export function loadMcpConfig(): McpConfig {
  ...
  for (const [name, server] of Object.entries(parsed.servers)) {
    if (!/^[a-zA-Z0-9_-]+$/.test(name) || !server.trust) throw new Error(`Invalid MCP server configuration: ${name}`);
    ...
    if (transport === 'http') {
      ...
      if (url.protocol !== 'https:' && !['localhost', '127.0.0.1', '::1'].includes(url.hostname)) throw new Error(`MCP HTTP server ${name} must use HTTPS or loopback`);
    }
